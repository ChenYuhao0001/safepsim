...
try:
    import simulationcommon_3
    from simulationcommon_3 import vehicle_length
except ImportError:
    vehicle_length = 4.7
...
try:
    from simulationcommon_3 import common_simulation_parameters, get_value, construct_vector_x0, calculate_error
...
def detect_parameters_using_functions():
    ...
    x_vec = simulationcommon_3.construct_vector_x0(n_test, p0_test, v0_test, a0_test, h_test)

    p0_val = x_vec[0, 0]
    p1_val = x_vec[5, 0]

    detected_gap = p0_val - p1_val
    error_val = simulationcommon_3.calculate_error(1, x_vec, h_test, v0_test)
...
if __name__ == "__main__":
    ...
    kp_values = [0.1, 0.15, 0.2, 0.25]  #
    kd_values = [1.0, 1.25, 1.5]
    ...
    for idx, (kp, kd) in enumerate([(k, d) for k in kp_values for d in kd_values]):
        try:
            ...
            if idx == 0:
                ...
                batch_folder_name = f"Batch_Analysis_{timestamp}_Detected_r{detected_r_final:.1f}"
    ...
    if batch_folder_path:
        ...
        markers = {0.05: 'v', 0.10: '^', 0.15: 'D', 0.20: 's', 0.25: 'o'}
        colors = {0.5: 'blue', 0.75: 'cyan', 1.0: 'C0', 1.25: 'C1', 1.5: 'C2'}

        for r in summary_results:
            kp, kd, fuel, dmin, fuel_error = r
        ...
        plt.xlabel(r"Average Fuel Saved (All Vehicles) $\frac{1}{t_{\mathrm{end}}} \sum_{i=1}^{n}"
        ...
        plt.legend(by_label.values(), by_label.keys(), loc='upper left', bbox_to_anchor=(1.02, 1), fontsize='small',
                   borderaxespad=0.)
        summary_path = os.path.join(batch_folder_path, "Summary_Fuel_vs_Safety.pdf")
        ...
        if table_data:
            ...
            table_pdf_path = os.path.join(batch_folder_path, "Error_Analysis_Table.pdf")
