"""
Shared helpers for the three EJC revision figures.

Contains:
- setup_latex(): enable LaTeX rendering (falls back gracefully if unavailable).
- get_get_nuk_thm7(): the Theorem 7 (lifted-state) adaptive step-size rule,
  copied verbatim from histogram_5_SDP_2575.py so the figures use the same
  simulation approach as the paper. This does NOT modify any existing file.

All core numerical routines (matrix generation, Lambert-W t*, exact braking
solution, minimum distance) are imported from simulationcommon_11.py.
"""
