...
def generate_matrix_Ac(n, tau_d=1.5, h=0.6, k_p=0.2, k_d=0.7):
    ...
    for s2 in range(1, n + 1):
        ...
        if 0 <= row < Ac and 0 <= col < Ac: matrix[row, col] = -h
    for s3 in range(1, n + 1):
        row, col = -2 + 6 * s3, 1 + 6 * s3
        if 0 <= row < Ac and 0 <= col < Ac: matrix[row, col] = h / tau_d - 1
    for s4 in range(1, n + 1):
        row, col = -2 + 6 * s4, 2 + 6 * s4
        if 0 <= row < Ac and 0 <= col < Ac: matrix[row, col] = -h / tau_d
...
def construct_vector_x0(n, p0, v0, a0, h):
    x = np.matrix(np.zeros((3 + 6 * n, 1)))
    x[0, 0], x[1, 0], x[2, 0] = p0, v0, a0
    initial_gap = 5  # 5 original value
    for i in range(1, n + 1):
        index = 3 + (i - 1) * 6
        x[index + 2, 0] = 200 - (initial_gap + h * v0) * i
        x[index + 3, 0], x[index + 4, 0], x[index + 5, 0] = v0, a0, 0
        x[index, 0] = calculate_error(i, x, h, v0)
        if i > 1:
            x[index + 1, 0] = x[index - 3, 0] - x[index + 3, 0] - h * a0
        else:
            x[index + 1, 0] = x[1, 0] - x[index + 3, 0] - h * a0
    return x
...
def calculate_error(i, x, h, v0):
    p_i = x[0, 0] if i == 1 else x[-1 + 6 * (i - 1), 0]
    j_index = 5 + 6 * (i - 1)
    length, r = 4.7, 5  # 4.7 5 original values
    if j_index < x.shape[0]:
        p_j = x[j_index, 0]
        return p_i - p_j - length - r - h * v0
    else:
        return 0
...
def get_discrete_ab(n, tau_d, h, k_p, k_d, Nbar, T):
    filenamea = f"matrices/Amatrices_Nbar_{Nbar}_T_{T:.3f}_kp_{k_p:.3f}_kd_{k_d:.3f}.npy"
    filenameb = f"matrices/Bmatrices_Nbar_{Nbar}_T_{T:.3f}_kp_{k_p:.3f}_kd_{k_d:.3f}.npy"
    ...
    for k in range(kend - 1):
        ...
        if discrete_step >= break_step:
            ...
            if discrete_step < t_star_in_steps:
                ...
                uks[0, k] = calculate_u0_post_t_star((T/Nbar) * discrete_step, (T/Nbar) * t_star_in_steps, a0_at_t_star, v0_at_t_star, gamma, eta, tau_d)
...
def simulate_with_cache(simulation_parameters, get_get_nuk, description):
    ...
    if os.path.exists(dk_filename):
        ...
        # data = np.load(xks_filename)
        # if 'arr_0' in data:
        #     xks = np.matrix(data['arr_0'])
        # else:
