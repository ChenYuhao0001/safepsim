...
def get_discrete_ab(n, tau_d, h, k_p, k_d, Nbar, T):
    filenamea = (
        f"matrices/CTG_LPF_Amatrices_Nbar_{Nbar}_T_{T:.3f}"
        f"_tau_{tau_d:.3f}_h_{h:.3f}"
        f"_kp_{k_p:.4f}_kd_{k_d:.4f}"
    )

    filenameb = (
        f"matrices/CTG_LPF_Bmatrices_Nbar_{Nbar}_T_{T:.3f}"
        f"_tau_{tau_d:.3f}_h_{h:.3f}"
        f"_kp_{k_p:.4f}_kd_{k_d:.4f}"
    )
...
def simulate(simulation_parameters, get_get_nuk):
    ...
    # --- ADDED: State variable for Oscillation ---
    # True means "currently decreasing velocity towards 5"
    # False means "currently increasing velocity towards 30"
    slowing_down = True
    # ---------------------------------------------
