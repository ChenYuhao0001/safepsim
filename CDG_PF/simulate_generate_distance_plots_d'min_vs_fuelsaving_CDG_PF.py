...
try:
    import simulationcommon_3_CDG_PF
    from simulationcommon_3_CDG_PF import vehicle_length
except ImportError:
    vehicle_length = 4.7
...
try:
    from simulationcommon_3_CDG_PF import common_simulation_parameters, get_value, construct_vector_x0, calculate_error
...
def detect_parameters_using_functions():
    ...
    x_vec = simulationcommon_3_CDG_PF.construct_vector_x0(n_test, p0_test, v0_test, a0_test, h_test)

    p0_val = x_vec[0, 0]
    p1_val = x_vec[5, 0]

    detected_gap = p0_val - p1_val
    error_val = simulationcommon_3_CDG_PF.calculate_error(1, x_vec, h_test, v0_test)
...
if __name__ == "__main__":
    ...
    for idx, (kp, kd) in enumerate([(k, d) for k in kp_values for d in kd_values]):
        try:
            ...
            if idx == 0:
                ...
                batch_folder_name = f"Batch_Analysis_{timestamp}_CDG_PF"
    ...
    if batch_folder_path:
        ...
        plt.legend(by_label.values(), by_label.keys(), loc='upper left', bbox_to_anchor=(1.02, 1), fontsize='small',
                   borderaxespad=0.)
        summary_path = os.path.join(batch_folder_path, "Summary_Fuel_vs_Safety_CDG_PF.pdf")
        ...
        if table_data:
            ...
            table_pdf_path = os.path.join(batch_folder_path, "Error_Analysis_Table_CDG_PF.pdf")
